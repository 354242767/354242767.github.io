...
相信到这里，对vue和node有一定了解的都可以自己实现接下来其他的业务了，之后主要会构建登录（session判断是否登录，拦截请求，响应给前端，前端重定向到登录页）以及登录后的index页面展示（包括登出操作等），和admin页面（后台的CURD操作及前台的分页及排序等等）这里就不做过多介绍，主要是将前后端串联起来，之后的逻辑反而没有那么困难。想要后续代码的可去在线github。
