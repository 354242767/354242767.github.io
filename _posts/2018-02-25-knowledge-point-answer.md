...
**js函数的执行原理**
- 每一个JavaScript函数都是Function对象的一个实例, 它有一个仅供JS引擎存取的内部属性[[Scope]].
- 函数被创建时，这个内部属性就会包含函数被创建的作用域中对象的集合，这个集合呈链式链接，被称为函数的作用域链 
- 执行流进入函数时，函数环境会被推入到环境栈中
- 函数执行前预编译产生了一个活动对象AO（Active Object）：根据this、参数、函数声明、变量声明、arguments组成AO，这个对象挂被挂载到函数EC作用域链的最前端 （优先级:函数声明 > 函数参数 > 变量声明）
- 最后执行中变量按作用域逐级向上查找直至全局对象，没有则报错：未定义

**js变量提升**
- 答此问题需介绍js的语言特性：弱类型、动态的、解释型的脚本语言（了解不同语言特性见下）。-->js代码执行:第一个步骤是解析，第二个步骤是执行。 
- 所谓解析就是会先通篇扫描所有的Js代码，然后把所有声明提升到顶端（作用域）。执行（略）
